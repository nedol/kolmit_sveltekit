module.exports = {
  log: (arg, that) => {
    let now = (Date.now() / 1000).toFixed(3);
    //console.log(now + ': ', arg);
    try {
  ...
  QueryMethod: function (protocol, options, postData, res, cb) {
    let http_;
    if (protocol === 'http') http_ = http;
    else if (protocol === 'https') http_ = https;
    let req = http_.request(options, function (res) {
  ...
  longTap: function (longTapCallback) {
    return this.map(function () {
      ...
      elm.ontouchend = (e) => {
        ...
        if (
          dt.getTime() - start <
          500 //window.user.onCallButtonClick(e);
        );
        else longTapCallback.call(elm);
      };
      elm.ontouchstart = (e) => {
        // Set timeout
        let dt = new Date();
