...
async function POST({ request, setHeaders, url }) {
  ...
  switch (q.func) {
    case "operator":
      ...
      return new Response(
        JSON.stringify({ rows, fields, err })
      );
    case "getusers":
      if (q.abonent) {
    ...
    case "adddep":
      ...
      try {
        ...
        if (rows[0]) {
          ...
          [rows, fields] = await pool.query(sql, vals);
          return new Response(
            JSON.stringify({ func: q.func, dep: users2[q.id + 1] })
          );
        }
      } catch (ex) {
        return new Response(JSON.stringify({ error: ex.message }));
    ...
    case "changedep":
      ...
      try {
        ...
        if (rows[0]) {
          ...
          [rows, fields] = await pool.query(sql, vals);
          return new Response(
            JSON.stringify({ dep: users2[ind], fields, err })
          );
        }
      } catch (ex) {
        return new Response(JSON.stringify({ error: ex.message }));
    ...
    case "remdep":
      ...
      try {
        ...
        if (rows[0]) {
          ...
          [rows, fields] = await pool.query(query, vals);
          return new Response(
            JSON.stringify({ users: users2, fields, err })
          );
        }
      } catch (ex) {
        return new Response(JSON.stringify({ error: ex.message }));
