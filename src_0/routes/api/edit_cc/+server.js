...
export async function POST({ request, setHeaders, url }) {
	...
	case 'addoper':    
		...
		sql = "SELECT *, (SELECT users FROM users WHERE operator=?) as users "+
	...
	case 'adddep':    
		...
		try{
			...
			if(rows[0]){
				...
				users[q.id+1] =  {"id": String(q.id+1), "alias":"",
...
function SendEmail(q, new_email){
	...
	em.SendMail("nedol.kolmit@gmail.com", (q.send_mail || new_email), {
